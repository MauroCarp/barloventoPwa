      ...
      <div class="box-body">
       ...
       <table class="table table-bordered table-striped dt-responsive tablas" width="100%">
        ...
        <?php
          ...
          $valor = NULL;
          $orden = 'fechaSalida';
          $datos = ControladorDatos::ctrMostrardatos($item, $valor,$orden);
          
          foreach ($datos as $key => $value){
            echo ' <tr>
                    <td>'.$value["hotelero"].'</td>
                    <td>'.$value["caravana"].'</td>
                    <td>'.str_replace("Ã“","Ó",$value["tropa"]).'</td>
                    <td>'.$value["raza"].'</td>
                    <td>'.$value["destinoVenta"].'</td>
                    <td>'.$value["consignatario"].'</td>
                    <td>'.$value["proveedor"].'</td>
                    <td>'.formatearFecha($value["fechaIngreso"]).'</td>
                    <td>'.formatearFecha($value["fechaSalida"]).'</td>
                  </tr>';
          }
